...
export default function ItemDetail() {
  ...
  // ... (keep all the existing functions: fetchItem, fetchMessages, handleSettleDown, etc.)
  ...
  const handleSettleDown = async () => {
    if (!item || !user || user.id !== item.user_id) {
      return;
    }
    ...
    try {
      ...
      // Create notification for finder if exists
      if (item.finder_id) {
        await supabase.from('notifications').insert({
          user_id: item.finder_id,
          item_id: item.id,
          type: 'item_returned',
          content: `The item "${item.name}" has been marked as returned by the owner.`
        });
      }

      toast.success('Item has been settled down successfully!');
      navigate('/settledowns');
    } catch (error) {
      console.error('Error settling down item:', error);
      toast.error('Failed to settle down item.');
    }
  };
        ...
        <div className="p-6">
          <div className="flex justify-between items-start mb-4">
            ...
            <div className="flex items-center space-x-4">
              ...
              {user?.id === item.user_id && item.status !== 'returned' && (
                <button
